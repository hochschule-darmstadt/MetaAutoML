...
class AdapterManagerAgent(IAbstractBlackboardAgent):
    ...
    def __init__(self, blackboard: "Blackboard", strategy_controller: "StrategyController", adapter_manager: "AdapterManager"):
        super().__init__(blackboard, strategy_controller, f"training-{adapter_manager.get_training_identifier()}-adapter-{adapter_manager.get_automl_name()}" )
        self.adapter_manager = adapter_manager

    def can_contribute(self) -> bool:
        return (
            self.get_state({}).get(self.agent_identifier) is None or
            self.get_state({}).get(self.agent_identifier) != self.adapter_manager.get_status_for_blackboard()
        )
        
    def do_contribute(self) -> None:
        run_metrics = self.adapter_manager.get_status_for_blackboard()
        self.update_state({ self.agent_identifier: run_metrics }, True)
        ...
        if not self.adapter_manager.is_running():
            ...
            self._log.info(f'Agent {self.agent_identifier} is no longer active ({run_metrics["status"]}), unregistering...')
            self.unregister()
