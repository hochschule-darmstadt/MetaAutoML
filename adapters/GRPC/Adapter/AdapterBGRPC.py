...
class StartAutoMlConfiguration(betterproto.Message):
    ...
    selected_ml_libraries: List[str] = betterproto.string_field(5)
    """List of activated ML librariesexample:[":keras_lib",":h2o_lib"]"""
...
@dataclass(eq=False, repr=False)
class TerminateAutoMlRequest(betterproto.Message):
    session_id: str = betterproto.string_field(1)
    """
    Unique session id used to identify a running training session within the
    adapter, generated by the adapter before starting the AutoML solution
...
@dataclass(eq=False, repr=False)
class TerminateAutoMlResponse(betterproto.Message):
    pass
...
class AdapterServiceStub(betterproto.ServiceStub):
    ...
    async def terminate_auto_ml(
        self,
        terminate_auto_ml_request: "TerminateAutoMlRequest",
        *,
        timeout: Optional[float] = None,
        deadline: Optional["Deadline"] = None,
        metadata: Optional["MetadataLike"] = None
        ...
        return await self._unary_unary(
            "/AdapterService/TerminateAutoML",
            terminate_auto_ml_request,
            TerminateAutoMlResponse,
            timeout=timeout,
            deadline=deadline,
            metadata=metadata,
...
class AdapterServiceBase(ServiceBase):
    ...
    async def terminate_auto_ml(
        self, terminate_auto_ml_request: "TerminateAutoMlRequest"
    ) -> "TerminateAutoMlResponse":
        raise grpclib.GRPCError(grpclib.const.Status.UNIMPLEMENTED)
    ...
    async def __rpc_terminate_auto_ml(
        self,
        stream: "grpclib.server.Stream[TerminateAutoMlRequest, TerminateAutoMlResponse]",
    ) -> None:
        request = await stream.recv_message()
        response = await self.terminate_auto_ml(request)
        await stream.send_message(response)
    ...
    def __mapping__(self) -> Dict[str, grpclib.const.Handler]:
        return {
            ...
            "/AdapterService/TerminateAutoML": grpclib.const.Handler(
                self.__rpc_terminate_auto_ml,
                grpclib.const.Cardinality.UNARY_UNARY,
                TerminateAutoMlRequest,
                TerminateAutoMlResponse,
            ),
        }
